...
const AllTask = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_BACK_URL}/api/getAllQuestion`);
                            ...
                            {question.map((data, i) => (
                                ...
                                >
                                    <Typography variant="h6" sx={{ margin: '12px' }}>
                                        <Typography variant="h5" sx={{ display: 'inline-block', color: colors.greenAccent[400] }}>Question {i + 1}:</Typography> {data.questionText}
                                    </Typography>
